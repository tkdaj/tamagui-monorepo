...
- `pnpm run scripts:supabase-types` — runs `supabase` CLI to generate `lib/db/src/types/database.gen.ts`. If credentials are not present it can write a placeholder (use `--force` to force placeholder).
...
- The authoritative DB TypeScript types live at `lib/db/src/types/database.gen.ts` (generated by the Supabase CLI).
- Workflow:
  ...
  2.  Commit the generated `lib/db/src/types/database.gen.ts` so CI and other devs use the same types.
